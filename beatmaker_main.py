...
CSV_INSTAGRAM_UPDATED = 'Beatmakers with Instagram updated.csv'
CSV_COUPON_CODES = 'Coupon Codes.csv'


BM_FOLDER = 'beatmaker_main_txt'
BM_ADDITIONAL_PERMALINK = 'additional_permalink'

BM_BEATMAKERS = 'beatmakers.txt'
BM_PERMALINKS = 'permalinks.txt'
BM_ADDITIONAL_FOLDER = 'beatmaker_additional_main_txt'
BM_ADDITIONAL_POSITION_IN_ADDITIONAL_PERMALINK = 'position_of_beatmakers_unique_for_additional_permalink_from_main.txt'
...
def generate_2nd_permalinks(driver):
	...
	with open(f'{BM_ADDITIONAL_FOLDER}/{BM_ADDITIONAL_POSITION_IN_ADDITIONAL_PERMALINK}', 'a') as f:
		f.write("%s\n" % datetime.now())
		f.write("%s\n\n" % url)
	print(f'Position saved to {BM_ADDITIONAL_FOLDER}/{BM_ADDITIONAL_POSITION_IN_ADDITIONAL_PERMALINK}. Now scrolling page...')
	...
	for rapper_profile in soup.find_all(class_="sound__header"):
		...
		if check_genre(all_genres, 2, RESCRAPE):
			...
			print(rapper_profile_url.attrs['href'], f"\tis added to {BM_ADDITIONAL_FOLDER}/{BM_ADDITIONAL_PERMALINK}")

	with open(f'{BM_ADDITIONAL_FOLDER}/{BM_ADDITIONAL_PERMALINK}', 'a') as additional_file:
...
def get_rapper_profile_urls_from_reposts(permalinks):
	...
	for permalink in permalinks:
		...
		with open(f'{BM_FOLDER}/{BM_BEATMAKERS}', 'a') as f:
			for item in rapper_urls:
				f.write("%s\n" % item)

		print(f"{format(len(rapper_urls))} rapper profile URLs are written into file {BM_FOLDER}/{BM_BEATMAKERS}.")



def get_rapper_details():

	filenameEmail = f"csv/{CSV_EMAIL_UPDATED}"
	filenameInstagram = f"csv/{CSV_INSTAGRAM_UPDATED}"
	...
	if not os.path.exists(f"{BM_FOLDER}/beatmakers_unique.txt"): # check if unique series of data exists

		if os.path.exists(f"{BM_FOLDER}/{BM_BEATMAKERS}"): # if not, to see if it can be created from duplicate series
			with open(f'{BM_FOLDER}/{BM_BEATMAKERS}') as f:
				for item in f:
			...
			with open(f'{BM_FOLDER}/{BM_PERMALINKS}') as f:
				for item in f:
			...
			print(f"Please make {BM_FOLDER}/{BM_BEATMAKERS} first!")

	else:
		with open(f'{BM_FOLDER}/beatmakers_unique.txt') as f:
			for item in f:
	...
	print('This will create a list of unique rappers and also look in /likes to get a boost in results number.')
...
def main(): # Main workflow of SoundCloud Scraper
	...
	if not os.path.exists(f"{BM_FOLDER}/{BM_PERMALINKS}"): # Searches if permalinks to repost profiles are already made
		...
		with open(f'{BM_FOLDER}/{BM_PERMALINKS}', 'w') as f: # Write permalinks of repost profiles to file
	...
	if not os.path.exists(f"{BM_FOLDER}/{BM_BEATMAKERS}"): # If rappers' profile urls are not scraped from repost profiles
