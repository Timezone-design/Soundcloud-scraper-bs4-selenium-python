...
import pandas as pd
from os import path
from resources import am_take_screenshot
from constants import SCREENSHOT_UPLOAD_URL, SLUG_FILTER_PATTERN
from datetime import datetime


filenameEmail = path.join(path.dirname(path.abspath(__file__)), 'audiomack', 'Rappers with Email.csv')
filenameCoupon = path.join(path.dirname(path.abspath(__file__)), 'am_csv', 'Coupon Codes.csv')
filenameFinal = path.join(path.dirname(path.abspath(__file__)), 'am_csv', 'Rappers with Email final.csv')
filenamePos = path.join(path.dirname(path.abspath(__file__)), 'screenshot_txt', 'am_position_of_screenshot_for_filenameCoupon.txt')
filenameLog = path.join(path.dirname(path.abspath(__file__)), 'screenshot_txt', 'am_screenshot_log.csv')
...
if emaildfEncoding == '':
  try:
    emaildf = pd.read_csv(filenameEmail, encoding='utf-8', index_col=False, header=0, error_bad_lines=False, sep=',')
...
if emaildfEncoding == '':
  try:
    emaildf = pd.read_csv(filenameEmail, encoding='utf-16', index_col=False, header=0, error_bad_lines=False, sep='\t')
...
if emaildfEncoding == '':
  try:
    emaildf = pd.read_csv(filenameEmail, encoding='ISO-8859-1', index_col=False, header=0, error_bad_lines=False, sep=',')
    emaildfEncoding = 'ISO-8859-1'
  except:
    pass

coupondf = pd.read_csv(filenameCoupon, encoding='utf-8-sig', index_col=False, header=0, error_bad_lines=False, sep=',')
...
if len(newlines.index) > 0:
  ...
  newlines_all.drop(['UserName', 'FullName', 'ArtistName', 'ArtistNameCleaned', 'Location', 'Country', 'Email', 'HasInstagram', 'SongTitle', 'SongTitleFull', 'GO+', 'SongLink', 'Genre', 'ArtistOrManager', 'NumberOfFollowers', 'Popularity', 'CouponCodeName', 'CouponCode', 'SongPlays', 'UploadDate', 'PopularityAdjusted', 'ActiveState', 'ScreenshotFileName', 'ScreenshotURL', 'InLosAngeles'], axis=1).to_csv(filenameLog, mode='a', header=False, encoding='utf-8', sep=',', index=False)
  print('File log made for screenshot waiting list')
  
  for index, url in enumerate(newlines_all['ArtistOrManager']):
    ...
    # filename = 'None'
    filename = am_take_screenshot(url, str(newlines_all.iloc[index]['SoundCloudURL']).rsplit('/')[-1], str(newlines_all.iloc[index]['GO+']).rsplit()[0])
    if filename != 'None':
